                            ...
                            tabPanel("Automatic", value = "automatic",
                                     div(class = "dashboard-card", style = "margin-top: 20px;",
                                         ...
                                         div(class = "dashboard-card__content",
                                             ...
                                             actionButton("notable_changes", "Notable Changes", class = "govuk-button govuk-button--blue"),
  ...
  div(class = "govuk-width-container govuk-width-container--wide",
      tags$main(class = "govuk-main-wrapper", style = "padding-top: 0;",
                ...
                div(class = "dashboard-card",
                    ...
                    div(class = "dashboard-card__content", uiOutput("notable_preview"))
                )
      )
  ),
...
server <- function(input, output, session) {
  ...
  .detect_oecd_type <- function(nm, path = NULL) {
    ...
    # Filename no't help — inspect file contents
    if (!is.null(path)) {
  ...
  ons_download_urls <- list(
    A01   = "https://www.ons.gov.uk/file?uri=/employmentandlabourmarket/peopleinwork/employmentandemployeetypes/datasets/summaryoflabourmarketstatistics/current/a01mar2026.xls",
    X09   = "https://www.ons.gov.uk/file?uri=/employmentandlabourmarket/peopleinwork/earningsandworkinghours/datasets/x09realaverageweeklyearningsusingconsumerpriceinflationseasonallyadjusted/march2026/x09mar2026.xlsx",
    RTISA = "https://www.ons.gov.uk/file?uri=/employmentandlabourmarket/peopleinwork/earningsandworkinghours/datasets/realtimeinformationstatisticsreferencetableseasonallyadjusted/current/rtisamar2026.xlsx"
  )
  ...
  output$manual_download_excel <- downloadHandler(
    filename = function() {
      mm <- reference_manual_month()
      label <- if (!is.null(mm) && nzchar(mm)) manual_month_to_display(mm) else format(Sys.Date(), "%B %Y")
      paste0("A01 - ", label, ".xlsx")
    },
    content = function(file) {
      if (is.null(uploaded_files$a01)) {
        showNotification("Upload the A01 file first", type = "warning")
        return()
      }
      # Serve the uploaded file straight back — identical bytes, perfect copy.
      file.copy(uploaded_files$a01, file, overwrite = TRUE)
      showNotification("A01 downloaded", type = "message", duration = 3)
    }
  )
  ...
  # --- Notable Changes (Automatic) ---

  observeEvent(input$notable_changes, {

    withProgress(message = "Analysing Notable Changes", value = 0, {

      incProgress(0.1, detail = "Checking configuration...")
      ...
      incProgress(0.2, detail = "Detecting notable changes...")

      nc <- if (exists("notable_changes", envir = calc_env)) {
        get("notable_changes", envir = calc_env)
      } else {
        data.frame(narrative = character(), stringsAsFactors = FALSE)
      }
    ...
    showNotification("Notable changes loaded", type = "message", duration = 3)
  })

  # --- Notable Changes (Manual) ---

  observeEvent(input$manual_notable_changes, {
    if (!.check_manual_ready()) return()

    withProgress(message = "Analysing Notable Changes (Manual)", value = 0, {

      incProgress(0.2, detail = "Running Excel calculations...")

      if (file.exists(config_path)) source(config_path, local = FALSE)
      source("utils/helpers.R", local = FALSE)
      source("utils/significance.R", local = FALSE)
      ...
      incProgress(0.5, detail = "Detecting notable changes...")

      # Build the lfs list from global variables set by Excel calculations
      lfs_from_excel <- tryCatch({
        .gv <- function(name) if (exists(name, inherits = TRUE)) get(name, inherits = TRUE) else NA_real_
        list(
          emp16        = list(cur = .gv("emp16_cur"),       dq = .gv("emp16_dq"),       dy = .gv("emp16_dy"),       dc = .gv("emp16_dc"),       de = .gv("emp16_de")),
          emp_rt       = list(cur = .gv("emp_rt_cur"),      dq = .gv("emp_rt_dq"),      dy = .gv("emp_rt_dy"),      dc = .gv("emp_rt_dc"),      de = .gv("emp_rt_de")),
          unemp16      = list(cur = .gv("unemp16_cur"),     dq = .gv("unemp16_dq"),     dy = .gv("unemp16_dy"),     dc = .gv("unemp16_dc"),     de = .gv("unemp16_de")),
          unemp_rt     = list(cur = .gv("unemp_rt_cur"),    dq = .gv("unemp_rt_dq"),    dy = .gv("unemp_rt_dy"),    dc = .gv("unemp_rt_dc"),    de = .gv("unemp_rt_de")),
          inact        = list(cur = .gv("inact_cur"),       dq = .gv("inact_dq"),       dy = .gv("inact_dy"),       dc = .gv("inact_dc"),       de = .gv("inact_de")),
          inact_rt     = list(cur = .gv("inact_rt_cur"),    dq = .gv("inact_rt_dq"),    dy = .gv("inact_rt_dy"),    dc = .gv("inact_rt_dc"),    de = .gv("inact_rt_de")),
          inact5064    = list(cur = .gv("inact5064_cur"),   dq = .gv("inact5064_dq"),   dy = .gv("inact5064_dy"),   dc = .gv("inact5064_dc"),   de = .gv("inact5064_de")),
          inact5064_rt = list(cur = .gv("inact5064_rt_cur"),dq = .gv("inact5064_rt_dq"),dy = .gv("inact5064_rt_dy"),dc = .gv("inact5064_rt_dc"),de = .gv("inact5064_rt_de"))
        )
      }, error = function(e) NULL)

      nc <- tryCatch(find_notable_changes(lfs_from_excel), error = function(e) {
        showNotification(paste("Notable changes error:", e$message), type = "error")
        data.frame(narrative = character(), stringsAsFactors = FALSE)
      })
    ...
    showNotification("Notable changes loaded (Manual)", type = "message", duration = 3)
  })
  ...
  output$notable_preview <- renderUI({
    ...
    if (is.null(nc)) {
      return(p(class = "govuk-body",
               "Click 'Notable Changes' to identify statistically significant LFS movements."))
    }

    if (nrow(nc) == 0) {
      return(div(
        tags$span(class = "govuk-tag govuk-tag--grey", "None"),
        p(class = "govuk-body", style = "margin-top: 10px;",
          "No statistically significant changes detected at the 95% confidence level.")
      ))
    }

    items <- lapply(seq_len(nrow(nc)), function(i) {
      row <- nc[i, ]
      div(style = "padding: 12px; margin-bottom: 8px; background-color: #ffffff; border-left: 4px solid #00703c;",
        div(style = "margin-bottom: 4px;",
          tags$span(class = "govuk-tag govuk-tag--green", "Significant"),
          tags$span(style = "margin-left: 8px; font-weight: 700; font-size: 14px;",
                    paste0(row$metric_label, " \u2014 ", row$comparison_label))
        ),
        p(class = "govuk-body", style = "margin: 0; font-size: 15px;", row$narrative),
        tags$span(style = "font-size: 12px; color: #505a5f;",
                  paste0("z = ", format(round(row$z_score, 2), nsmall = 2)))
      )
    })

    tagList(
      p(class = "govuk-body", style = "margin-bottom: 15px; color: #505a5f;",
        paste0("Showing ", nrow(nc), " statistically significant change",
               if (nrow(nc) != 1) "s", " (ONS 95% CI methodology, ranked by strength of evidence)")),
      div(items)
    )
  })
}
