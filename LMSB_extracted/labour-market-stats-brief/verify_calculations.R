...
# KNOWN-GOOD VALUES
# =============================================================================
# These are the values from the published Feb 2026 Labour Market Stats Briefing.
# Update this list each month after manually verifying the published briefing.
...
known_good_feb2026 <- list(
  ...
  # LFS levels (Sheet 1, in thousands as stored)
  # emp16_cur      = 34244,    # Employment 16+ (uncomment when verified)
  # unemp16_cur    = 1883,     # Unemployment 16+ (uncomment when verified)
  # inact_cur      = 9353,     # Inactivity 16-64 (uncomment when verified)

  # Inactivity 50-64 (Sheet 2)
  # inact5064_rt_cur = 25.8,   # (uncomment when verified)
...
verify_all <- function(manual_month, file_a01 = NULL, file_hr1 = NULL,
  ...
  # Helper: get variable from calc_env safely
  sv <- function(name) {
    if (exists(name, envir = calc_env)) get(name, envir = calc_env) else NA_real_
  }

  # Check each known-good value
